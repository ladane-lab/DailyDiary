...
export async function performMaintenanceCleanup(): Promise<void> {
  ...
  try {
    ...
    // 3. Enforce TrustedDevice cap of 10 per user (sweeps all users to enforce limit)
    ...
    if (evictedDeviceCount > 0) {
      logger.info(`[Cleanup] Enforced device limit: Evicted ${evictedDeviceCount} old trusted devices across active accounts.`);
    }

    logger.info('[Cleanup] Database maintenance completed successfully.');
  } catch (error: any) {
    logger.error('[Cleanup] Error executing database maintenance task:', error);
  }
}
